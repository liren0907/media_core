...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pipeline = Pipeline::new()
        .add_node(ExtractFrames::new(SamplingStrategy::FirstN(10)));
    ...
    let pipeline = Pipeline::new()
        .add_node(ExtractFrames::new(SamplingStrategy::Range(0, 20)));
    ...
    let pipeline = Pipeline::new()
        .add_node(ExtractFrames::new(SamplingStrategy::KeyFrames));
    ...
    let pipeline = Pipeline::new()
        .add_node(ExtractFrames::new(SamplingStrategy::Custom(vec![0, 50, 100])));
    ...
    // Full size (no scale)
    let pipeline = Pipeline::new()
        .add_node(ExtractFrames::new(SamplingStrategy::FirstN(1)));
    let context = MediaContext::from_file(Path::new(video_path).to_path_buf());
    let full = pipeline.execute(context)?;
    let full_size = full.extracted_frames.first().map(|f| f.data.len()).unwrap_or(0);

    // Half size (0.5)
    let pipeline = Pipeline::new()
        .add_node(ExtractFrames::new(SamplingStrategy::FirstN(1)).with_scale(0.5));
    let context = MediaContext::from_file(Path::new(video_path).to_path_buf());
    let half = pipeline.execute(context)?;
    let half_size = half.extracted_frames.first().map(|f| f.data.len()).unwrap_or(0);

    // Quarter size (0.25)
    let pipeline = Pipeline::new()
        .add_node(ExtractFrames::new(SamplingStrategy::FirstN(1)).with_scale(0.25));
    let context = MediaContext::from_file(Path::new(video_path).to_path_buf());
    let quarter = pipeline.execute(context)?;
    let quarter_size = quarter.extracted_frames.first().map(|f| f.data.len()).unwrap_or(0);
